...
void EV_Cannon(struct event_args_s *args)
{
	...
	if (EV_IsLocal(idx))
	{
		switch (firestate)
		{
		case 0:  gEngfuncs.pEventAPI->EV_WeaponAnimation(CANNON_FIRE, 0); break;
		default: break;
		}
	}
	...
	case 2: gEngfuncs.pEventAPI->EV_PlaySound(idx, origin, CHAN_WEAPON, "weapons/cannon_fire1.wav", 1, ATTN_NORM, 0, 100); break;
	}
}
...
void EV_Colts(struct event_args_s *args)
{
	...
	quickfire = args->bparam1;
	leftattack = args->bparam2;
	AngleVectors(angles, forward, right, up);
	...
	{
		EV_MuzzleFlash();
		gEngfuncs.pEventAPI->EV_WeaponAnimation(((!quickfire) ? ((leftattack) ? COLTS_LEFTFIRE : COLTS_RIGHTFIRE) : COLTS_DUALFIRE), 0);
	...
	switch (gEngfuncs.pfnRandomLong(0, 1))
	{
	case 0:	gEngfuncs.pEventAPI->EV_PlaySound(idx, origin, CHAN_WEAPON, "weapons/coltsfire1.wav", 1, ATTN_NORM, 0, 100);	break;
	case 1:	gEngfuncs.pEventAPI->EV_PlaySound(idx, origin, CHAN_WEAPON, "weapons/coltsfire2.wav", 1, ATTN_NORM, 0, 100);	break;
	}
...
void EV_Pick(struct event_args_s *args)
{
	...
	//Play Swing sound
	gEngfuncs.pEventAPI->EV_PlaySound(idx, origin, CHAN_WEAPON, "weapons/pick_miss1.wav", 1, ATTN_NORM, 0, PITCH_NORM);

	if (EV_IsLocal(idx))
	{
		gEngfuncs.pEventAPI->EV_WeaponAnimation(PICK_ATTACK1MISS, 1);

		switch ((g_iSwing++) % 3)
		{
		case 0:
			gEngfuncs.pEventAPI->EV_WeaponAnimation(PICK_ATTACK1MISS, 1); break;
		case 1:
			gEngfuncs.pEventAPI->EV_WeaponAnimation(PICK_ATTACK2MISS, 1); break;
		case 2:
			gEngfuncs.pEventAPI->EV_WeaponAnimation(PICK_ATTACK3MISS, 1); break;
		}
	}
}
...
void EV_Pistol(struct event_args_s *args)
{
	...
	{
		EV_MuzzleFlash();
		gEngfuncs.pEventAPI->EV_WeaponAnimation(quickfire ? PISTOL_QUICKFIRE_SHOOT : PISTOL_SHOOT, 0);
	...
	switch (gEngfuncs.pfnRandomLong(0, 1))
	{
	case 0:	gEngfuncs.pEventAPI->EV_PlaySound(idx, origin, CHAN_WEAPON, "weapons/pistol_shot1.wav", 1, ATTN_NORM, 0, 100);	break;
	case 1:	gEngfuncs.pEventAPI->EV_PlaySound(idx, origin, CHAN_WEAPON, "weapons/pistol_shot2.wav", 1, ATTN_NORM, 0, 100);	break;
	}
...
void EV_Shotgun(struct event_args_s *args)
{
	...
	if (EV_IsLocal(idx))
	{
		...
		if (!secondaryfire)
		{
			gEngfuncs.pEventAPI->EV_WeaponAnimation(DBARREL_SHOOT, 2);

			V_PunchAxis(0, -5.0);
		}
		else
		{
			gEngfuncs.pEventAPI->EV_WeaponAnimation(DBARREL_SHOOT2, 2);

			V_PunchAxis(0, -10.0);
		}
	}

	for (int i = 0; i < args->iparam1; i++)
	{
		EV_GetDefaultShellInfo(args, origin, velocity, ShellVelocity, ShellOrigin, forward, right, up, 32, -12, 6);
...
void EV_Winchester(struct event_args_s *args)
{
	...
	if (EV_IsLocal(idx))
	{
		// Add muzzle flash to current weapon model
		EV_MuzzleFlash();
		gEngfuncs.pEventAPI->EV_WeaponAnimation(WINCHESTER_FIRE, 2);
	...
	switch (gEngfuncs.pfnRandomLong(0, 2))
	{
	case 0: gEngfuncs.pEventAPI->EV_PlaySound(idx, origin, CHAN_WEAPON, "weapons/winchester_fire1.wav", gEngfuncs.pfnRandomFloat(0.95, 1.0), ATTN_NORM, 0, 93 + gEngfuncs.pfnRandomLong(0, 0x1f)); break;
	case 1: gEngfuncs.pEventAPI->EV_PlaySound(idx, origin, CHAN_WEAPON, "weapons/winchester_fire2.wav", gEngfuncs.pfnRandomFloat(0.95, 1.0), ATTN_NORM, 0, 93 + gEngfuncs.pfnRandomLong(0, 0x1f)); break;
	case 2: gEngfuncs.pEventAPI->EV_PlaySound(idx, origin, CHAN_WEAPON, "weapons/winchester_fire3.wav", gEngfuncs.pfnRandomFloat(0.95, 1.0), ATTN_NORM, 0, 93 + gEngfuncs.pfnRandomLong(0, 0x1f)); break;
	}
...
void EV_Buffalo(event_args_t *args)
{
	int idx;
	vec3_t origin;
	vec3_t angles;
	...
	if (EV_IsLocal(idx))
	{
		...
		gEngfuncs.pEventAPI->EV_WeaponAnimation(BUFFALO_FIRE, 2);
	...
	{
	case 0:
		gEngfuncs.pEventAPI->EV_PlaySound(idx, origin, CHAN_WEAPON, "weapons/buffalo_shoot1.wav", gEngfuncs.pfnRandomFloat(0.8, 0.9), ATTN_NORM, 0, PITCH_NORM);
		break;
	case 1:
		gEngfuncs.pEventAPI->EV_PlaySound(idx, origin, CHAN_WEAPON, "weapons/buffalo_shoot2.wav", gEngfuncs.pfnRandomFloat(0.8, 0.9), ATTN_NORM, 0, PITCH_NORM);
		break;
	}
...
void EV_Gattlinggun(event_args_t *args)
{
...
void EV_Scorpion(event_args_t *args)
{
	int idx;
	vec3_t vecSrc, angles, view_ofs, forward;
	pmtrace_t tr;
...
void EV_Beartrap(event_args_t *args)
{
	int idx;
	vec3_t vecSrc, angles, view_ofs, forward;
	pmtrace_t tr;
