...
enum { HEAD_GLASSES = 0, HEAD_EINSTEIN = 1, HEAD_LUTHER = 2, HEAD_ROSENBERG = 3 };
#else
...
#if defined ( BSHIFT_UNLOCKED_DLL )
	int	FOkToSpeak(void);
	BOOL IsRosenberg() { return FClassnameIs(pev, "monster_rosenberg"); }
	static const char* pScientistPainSounds[];
	static const char* pRosenbergPainSounds[];
#endif // defined ( BSHIFT_UNLOCKED_DLL )
...
const char* CScientist::pScientistPainSounds[] =
{
...
const char* CScientist::pRosenbergPainSounds[] =
{
...
void CScientist :: Precache( void )
{
	...
	if (IsRosenberg())
	{
		PRECACHE_SOUND_ARRAY(pRosenbergPainSounds);
	}
	else
	{
		PRECACHE_SOUND_ARRAY(pScientistPainSounds);
	}
#else
...
void CScientist :: TalkInit()
{
	...
	if (IsRosenberg())
	{
		...
		// Rosenberg uses a special head submodel.
...
#if defined ( BSHIFT_UNLOCKED_DLL )
	case HEAD_ROSENBERG: m_voicePitch = 100;  break;//rosenberg
#else
...
{
#if defined ( BSHIFT_UNLOCKED_DLL )
	if ( !IsRosenberg() && pevInflictor && pevInflictor->flags & FL_CLIENT )
#else
...
#if defined ( BSHIFT_UNLOCKED_DLL )
	if ( IsRosenberg() )
		EMIT_SOUND_DYN( ENT(pev), CHAN_VOICE, RANDOM_SOUND_ARRAY(pRosenbergPainSounds), 1, ATTN_NORM, 0, GetVoicePitch());
	else
		EMIT_SOUND_DYN( ENT(pev), CHAN_VOICE, RANDOM_SOUND_ARRAY(pScientistPainSounds), 1, ATTN_NORM, 0, GetVoicePitch());
#else
...
#if defined ( BSHIFT_UNLOCKED_DLL )
int	CScientist::FOkToSpeak(void)
{
	if ( IsRosenberg() )
		return FALSE;
